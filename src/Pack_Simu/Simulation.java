 ...
 */
			...
			ArrayList<Client> clientAlgoList, int clientWaitingNumber)
	{
		...
		int costMin = cost(matriceDePassage,carAlgoList,clientAlgoList); //On cherche le coût minimum des matrices de passages créées
	 ...
	 */
	public void OneMove() {
		this.time = this.time+1; //On incrémente le temps de la simulation
		int activeCar = 0; //Nombre de voiture en circulation
		int speedSum = 0; //Somme des vitesses des voitures
		double clientRealSpeedSum = 0; //Somme des vitesses instantées des clients
		this.model.setClientSpeedSum(0); //Met la somme ?? à  0
...
/_/    \_\ |\___/ \__,_|\__| |_____/ \__,_| .__/| .__/|_|  |_|_| |_| |_|\___|
	...
	public int cost(int[][] matriceDePassage, ArrayList<Car> carAlgoList, ArrayList<Client> clientAlgoList)
	{
		...
		for(int k=0;k<carAlgoNumber;k++)
		{
			...
			int bestDistSum = 0;
			int carDist = 0;
			int rank = 0; //Numéro d'ordre de l'étape de la voiture.
			Point p0 = null;
			int q1;
			
			while((q1 = searchInArray(rank,matriceDePassage[k]))!=-1)//On cherche la rankiême étape
			{
				
				Point p1 = clientAlgoList.get(q1/2).getPosClient()[q1%2]; //p1 point correspondant à q1
				...
				rank++; //On passe à l'étape suivante
				p0 = p1;
			}
