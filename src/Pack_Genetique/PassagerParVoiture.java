  ...
  | |___| (_) | | | \__ \ |_| |  | |_| | (__| ||  __/ |_| | |   
   \_____\___/|_| |_|___/\__|_|   \__,_|\___|\__\___|\__,_|_|  
	...
	public PassagerParVoiture() {
		this.passagersOrdonnes = repartitionAleatoirePassager();
		
		this.passageOptimal = new PositionPassager[Main.nbVoiture][Main.nbPlaceVoiture*2];
		
		this.pointsDePassage = this.attribuerPointsDePassage(); 
	 ...
	 * @since Build III -  v0.0
     */
    public Passager[][] repartitionAleatoirePassager() {
    	/* Initialisations */
		int index = 0;
		Passager[] listeDesPassagers= new Passager[Main.nbPassager];
		Passager[][] tab = new Passager[Main.nbVoiture][Main.nbPlaceVoiture];
		
		/* Création d'une liste aléatoire de passager */
		...
		// affection des passagers dans les voitures : 
		for(int i = 0 ; i < Main.nbVoiture ; i++) {
			for(int j = 0 ; j < Main.nbPlaceVoiture ; j++){
				tab[i][j] = listeDesPassagers[index];
				index++;
			}
		}
	 ...
	 * @since Build III -  v0.0
	 */
    public PositionPassager[][] attribuerPointsDePassage() {
    	PositionPassager[][] tab =  new PositionPassager[Main.nbVoiture][Main.nbPlaceVoiture*2];
    	for (int i = 0; i < Main.nbVoiture; i++){ // Pour chaque voiture ayant x Place
    		Pre_Algo_DeterministePourPoint(passagersOrdonnes[i], i);
    		for(int j = 0; j < this.passageOptimal[i].length ; j++) {
    			tab[i][j] = this.passageOptimal[i][j];
    		}
    		this.passageOptimal = new PositionPassager[Main.nbVoiture][Main.nbPlaceVoiture*2];
    	}
    	return tab;
