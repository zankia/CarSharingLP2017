            ...
            if (this.win.realTime ? true : !this.win.found && !this.win.searching) {
                ...
                if (win.typeColoriage) {
                    if (evt.getButton() == MouseEvent.BUTTON1
                        && !(st == States.START_CLIENT || st == States.END_CLIENT)) {
                    ...
                    } else if (st == States.VOID && !(st == States.START_CLIENT
                        || st == States.END_CLIENT)) {
    ...
    public void mouseDragged(MouseEvent evt) {
        ...
        if (row >= 0 && row < this.win.rows && col >= 0 && col < this.win.columns) {
            if (this.win.realTime ? true : !this.win.found && !this.win.searching) {
                ...
                if ((row*this.win.columns+col != row*this.win.columns+col)
                    && (cur_val == States.CAR || cur_val == States.START_CLIENT
                    || cur_val == States.END_CLIENT)) {
                ...
                } else if (this.win.grid[row][col].getStates() != States.WALL
                    && this.win.grid[row][col].getStates() != States.CAR
                    && this.win.grid[row][col].getStates() != States.START_CLIENT
                    && this.win.grid[row][col].getStates() != States.END_CLIENT) {
                        this.win.grid[row][col].setStates(States.WALL);
                        win.list_block.add(this.win.grid[row][col]);
                }
            }
