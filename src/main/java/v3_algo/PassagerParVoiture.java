...
public class PassagerParVoiture {
   ...
   public void afficherPassagerOnVoitures() {
            ...
            for (int j = 0; j < this.pointsDePassage[i].length - 1; j++) {
                ...
                this.open = new PriorityQueue<>((o1, o2) -> {
                    ...
                    return c1.finalCost < c2.finalCost ?
                        -1 : c1.finalCost > c2.finalCost ? 1 : 0;
                    });
                for (int a = 0; a < Execut_Algo_Genetique.sizeGrille_X; ++a) {
                    for (int b = 0; b < Execut_Algo_Genetique.sizeGrille_Y; ++b) {
                        ...
                        int k = Math.abs(a - this.pointsDePassage[i][j+1].getRow())
                            + Math.abs(b - this.pointsDePassage[i][j+1].getColumn());
                        Execut_Algo_Genetique.grid[a][b].heuristicCost = k;
                        Execut_Algo_Genetique.grid[a][b].parent = null;
                    }
                }
                Execut_Algo_Genetique.grid[this.pointsDePassage[i][j].getRow()]
                    [this.pointsDePassage[i][j].getColumn()].finalCost = 0;

                //add the start location to open list.
                this.open.add(Execut_Algo_Genetique.grid[this.pointsDePassage[i][j].getRow()]
                    [this.pointsDePassage[i][j].getColumn()]);
                ...
                while (true) {
                    ...
                    if (current.equals(Execut_Algo_Genetique.grid
                        [this.pointsDePassage[i][j+1].getRow()]
                        [this.pointsDePassage[i][j+1].getColumn()])) {
                        break;
                    }
                    Cell t;
                    if (current.row - 1 >= 0) {
                ...
                if (closed[this.pointsDePassage[i][j+1].getRow()]
                    [this.pointsDePassage[i][j+1].getColumn()]) {
                    //Trace back the path
                    Cell current2 = Execut_Algo_Genetique.grid
                        [this.pointsDePassage[i][j+1].getRow()]
                        [this.pointsDePassage[i][j+1].getColumn()];
    ...
    private void checkAndUpdateCost(Cell current, Cell t, int cost) {
        if (t.getStates() == States.WALL || this.closed[t.row][t.col]) {
            return;
        }
    ...
    public int getDistanceCheminPourU(Cell[] listeDesPoints, int nbPassager) {
        ...
        for (int j = 0; j < listeDesPoints.length - 1; j++) {
            ...
            for (int a = 0; a < Execut_Algo_Genetique.sizeGrille_X; ++a) {
                for (int b = 0; b < Execut_Algo_Genetique.sizeGrille_Y; ++b) {
                    Execut_Algo_Genetique.grid[a][b].heuristicCost =
                        Math.abs(a - listeDesPoints[j+1].getRow())
                        + Math.abs(b - listeDesPoints[j+1].getColumn());
                    Execut_Algo_Genetique.grid[a][b].parent = null;
                }
            }
            ...
            while (open.size() > 0) {
                ...
                if (current.equals(Execut_Algo_Genetique.grid[listeDesPoints[j+1].getRow()]
                    [listeDesPoints[j+1].getColumn()])) {
                    break;
                }
                Cell t;
                if (current.row - 1 >= 0) {
